...
class DragContext:
    ...
    '''
    (read-only) The widget where the draggable was released.
    '''
...
def save_widget_state(widget, *, ignore_parent=False) -> dict:
    '''
    Copies and returns the values of sizing and positioning properties of the widget.
    '''
    w = widget.__self__
    getattr_ = getattr
...
class KXDragReorderBehavior:
    ...
    The number of them will be the maximum number of simultaneous drags the :class:`KXDragReorderBehavior`` can handle.
    This can be changed only when there is no ongoing drag in this widget.
    '''
    ...
    @staticmethod
    def __touch_move_filter(ud_key, inactive_spacers, collide_point, drag_classes, widget, touch):
        return (ud_key not in touch.ud) and inactive_spacers and collide_point(*touch.pos) and \
            (touch.ud.get("kivyx_drag_cls", None) in drag_classes)

    async def __listen_to_touch_move_events(self):
        on_touch_move = partial(ak.event, self, "on_touch_move", filter=partial(
            self.__touch_move_filter, self.__tracked_touches, self.__inactive_spacers,
            self.collide_point, self.drag_classes))
        async with ak.open_nursery() as nursery:
            while True:
                __, touch = await on_touch_move()
                nursery.start(self.__place_a_spacer_under_finger(touch))
    ...
    async def __listen_to_touch_down_events(self):
        ...
        async with ak.open_nursery() as nursery:
            while True:
                __, touch = await on_touch_down()
                nursery.start(self.__handle_a_potential_dragging_gesture(touch))
    ...
    async def __handle_a_potential_dragging_gesture(self, touch):
        ...
        if ud.get("kivyx_drag_cls", None) not in self.drag_classes:
            return
        if self.__inactive_spacers and self.collide_point(*touch.pos):
            await self.__place_a_spacer_under_drag(touch)
        else:
            del ud[self.__ud_key]


    async def __place_a_spacer_under_drag(self, touch):
